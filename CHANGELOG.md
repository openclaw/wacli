...
- Sync: add opt-in keepalive-failure stale detection for `sync --follow`, including forced reconnect, a `stale` NDJSON event, a private store `HEARTBEAT`, and `doctor --json` `last_activity_at`. (#278 - thanks @thedavidweng)
