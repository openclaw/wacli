...
- History sync chunks of type `RECENT` and `FULL`, which can hold tens of thousands of messages, are queued in `history_sync_queue` and stored in order by a background worker, so live messages, receipts, and the offline backlog keep flowing while a chunk is stored. The queue is kept in the store because whatsmeow acknowledges the notification on arrival: a chunk interrupted by the sync stopping stays queued and is stored by the next sync, and a chunk whose download fails three times is dropped with a `history_queue_dropped` warning. `--once` and `auth` wait for the queue before exiting. `INITIAL_BOOTSTRAP` and the other small chunk types are still stored in line, because the bootstrap carries the phone's unread state; queued chunks leave unread counts alone, since by the time they are stored those may have moved on.
