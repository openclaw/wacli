...
- A message that arrives but cannot be read is reported as an `undecryptable_message` warning naming the chat, the sender, and the message ID, so a chat that is missing messages never looks like a chat that was quiet. The warning's `recovery` field says what that message gets, because it is not the same on every path: `requested_from_primary` when no ciphertext for this device arrived at all and your own primary device, which always holds a copy, is asked for one straight away; `resend_requested` when decryption failed, where the sender is asked to resend and, on the first failure for that message ID, the primary device is asked as well a few seconds later; and `none` for content WhatsApp keeps from linked devices on purpose, such as view-once messages, which cannot be read here at all. Some decryption failures - a bot message whose message secret is missing - are only acknowledged and get no request, and the event carries nothing that separates them, so `resend_requested` means a copy was asked back where the failure allowed it, not that one will arrive.
