...
- A message that arrives but cannot be decrypted is re-requested from the sender and, if the sender does not resend it within a few seconds, from your own primary device, which always holds a copy. Each one is reported as an `undecryptable_message` warning naming the chat, the sender, and the message ID, so a chat that is missing messages never looks like a chat that was quiet.
