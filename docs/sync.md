...
- If whatsmeow reports an app-state LTHash mismatch, sync first attempts a full refresh for that collection. After a successful refresh, a later mismatch can trigger another full refresh. If the refresh fails, sync asks the primary device for an official recovery snapshot with a fresh timeout. The snapshot request remains limited to once per collection for that sync run, including after a failed request, to avoid a retry storm; sync logs the failure and keeps handling normal message/history events.
