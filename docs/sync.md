...
- A message that arrives but cannot be read is reported as an `undecryptable_message` warning naming the chat, the sender, and the message ID, so a chat that is missing messages never looks like a chat that was quiet. The warning's `recovery` field says what that message gets. `none` is content WhatsApp keeps from linked devices on purpose, such as a view-once message: nothing was lost and nothing will arrive. Everything else is `requested_if_possible`, which is a request made where the failure allows one, not a promise that a copy comes back: a message with no ciphertext for this device is asked of your own primary device at once, a message that failed to decrypt has the sender asked to resend and, on the first failure for that message ID, the primary device asked as well a few seconds later, and a bot message whose message secret is missing is only acknowledged and gets neither. The event does not say which of those it is - `is_unavailable` is also set for an encrypted group message whose sender key is missing, where ciphertext did arrive - so the warning names what is known and leaves the outcome open.
