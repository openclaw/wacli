...
- If whatsmeow reports an app-state LTHash mismatch, sync first attempts a full app-state sync. A successful full sync completes recovery without requesting a snapshot. If the full sync still reports an LTHash mismatch, sync asks the primary device for the official recovery snapshot once for that collection. Other full-sync or recovery failures are printed as warnings, and sync keeps handling normal message/history events.
