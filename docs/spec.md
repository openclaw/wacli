...
- optional `--stale-threshold` detects keepalive failures and force-reconnects when the last successful keepalive is older than the given duration; accepted values are `1s` through `3m` because whatsmeow auto-reconnects after 3 minutes of failed keepalives
