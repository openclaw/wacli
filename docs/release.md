...
The release workflow dispatches the `Update Formula` workflow in `steipete/homebrew-tap` after release artifacts are published when the tap token is configured. The tap workflow owns the formula-editing logic and updates the per-platform artifact URLs and SHA256 values in `Formula/wacli.rb`.
